...
- 触类旁通。我在正式写代码之前，查看过qwen的tokenizer，对于特殊字符有了深刻的理解，在预训练的时候，观察到了复读机现象，从而阴差阳错地进行了scaling law实验，还发现qwen的预训练模型本身也存在复读机现象，在dpo的loss中对大模型过拟合有了直观的认知。这些都激励着我，让我感觉做这个项目是值得的。
